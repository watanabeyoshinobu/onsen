...
### その他機能
* トップページの自動スライドアニメーション(素のJavascriptで実装)
...
トップページに ライブラリを使用しない素のJavaScriptを用いた自動スライドアニメーションを使い、各スーパー銭湯紹介にもJavaScriptを使うことにより「魅せる」に力を入れ、感想欄でスーパー銭湯や温泉の話を「楽しむ」ことで皆さんと「繋がる」を意識することができます。<br>
Vue.js × Ruby on Rails　による部分的なSPA化を実装しました。<br>
Terraform × ECS × CircleCI × GitHub　による、インフラのパイプラインを構築しました。
